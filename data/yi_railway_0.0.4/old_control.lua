...
function detect_fluidHandlers(train)
	for k,carriage in ipairs(train.carriages) do
		if carriage.type == "cargo-wagon" then
			...
			if #handlers > 0 then
				if handlers[1].name == "y_loco_access-pipe-in" then
					fill_fluidTanks(carriage, handlers[1])
				elseif handlers[1].name == "y_loco_access-pipe-out" then
					empty_fluidTanks(carriage, handlers[1])
				end
			end
		end
	end
...
function fill_fluidTanks(carriage, fluidHandler)
	local foundCarriage
	for k,tanker in ipairs(global.YRFT) do
		if tanker[1] == carriage then
			foundCarriage = tanker
			break
		end
	end
	if foundCarriage ~= nil then
		if foundCarriage[2].type == nil then
			if fluidHandler.fluidbox[1] ~= nil then
				foundCarriage[2].type = fluidHandler.fluidbox[1].type
				foundCarriage[2].amount = math.min(fluidHandler.fluidbox[1].amount, entityMaxVolumes[foundCarriage[1].name][1])
				foundCarriage[2].temperature = fluidHandler.fluidbox[1].temperature
				
				fluidHandler.fluidbox[1] = {
					["type"] = fluidHandler.fluidbox[1].type,
					["amount"] = fluidHandler.fluidbox[1].amount - foundCarriage[2].amount,
					["temperature"] = fluidHandler.fluidbox[1].temperature
				}
			end
		elseif foundCarriage[2].amount < entityMaxVolumes[foundCarriage[1].name][1] then
			if fluidHandler.fluidbox[1] ~= nil then
				if foundCarriage[2].type == fluidHandler.fluidbox[1].type then				
					local fillableTankerVolume = math.min(entityMaxVolumes[foundCarriage[1].name][1] - foundCarriage[2].amount, fluidHandler.fluidbox[1].amount, entityMaxVolumes[foundCarriage[1].name][1])
					foundCarriage[2].temperature = (fillableTankerVolume * fluidHandler.fluidbox[1].temperature + foundCarriage[2].amount * foundCarriage[2].temperature) / (fillableTankerVolume + foundCarriage[2].amount)
					foundCarriage[2].amount = foundCarriage[2].amount + fillableTankerVolume
...
function empty_fluidTanks(carriage, fluidHandler)
	local foundCarriage
	for k,tanker in ipairs(global.YRFT) do
		if tanker[1] == carriage then
			foundCarriage = tanker
	...
	if foundCarriage ~= nil then
		if foundCarriage[2].type ~= nil then
			if fluidHandler.fluidbox[1] == nil then
				fluidHandler.fluidbox[1] = {
						["type"] = foundCarriage[2].type,
						["amount"] = foundCarriage[2].amount,
						["temperature"] = foundCarriage[2].temperature
				}
				foundCarriage[2].amount = 0
			elseif foundCarriage[2].type == fluidHandler.fluidbox[1].type and foundCarriage[2].amount > 0 then
				local fillableHandlerVolume = math.min(entityMaxVolumes[fluidHandler.name][1] - fluidHandler.fluidbox[1].amount, foundCarriage[2].amount)
				fluidHandler.fluidbox[1] = {
						["type"] = fluidHandler.fluidbox[1].type,
						["temperature"] = (fluidHandler.fluidbox[1].amount * fluidHandler.fluidbox[1].temperature + fillableHandlerVolume * foundCarriage[2].temperature) / (fluidHandler.fluidbox[1].amount + fillableHandlerVolume),
						["amount"] = fluidHandler.fluidbox[1].amount + fillableHandlerVolume
				}
				foundCarriage[2].amount = foundCarriage[2].amount - fillableHandlerVolume
			end
		end
	end
end